...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_DEVICE_BROKER_xxx
namespace = Collection("device_broker")
namespace.configure(
    {
        "device_broker": {
            "nautobot_ver": "2.3.1",
            "project_name": "device-broker",
            "python_ver": "3.11",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.device_broker.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.device_broker.compose_http_timeout,
        "NAUTOBOT_VER": context.device_broker.nautobot_ver,
        "PYTHON_VER": context.device_broker.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.device_broker.project_name}",
        f'--project-directory "{context.device_broker.compose_dir}"',
    ]

    for compose_file in context.device_broker.compose_files:
        compose_file_path = os.path.join(context.device_broker.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.device_broker.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.device_broker.python_ver}...")
    docker_compose(context, command)


def _ensure_creds_env_file(context):
    """Ensure that the development/creds.env file exists."""
    if not os.path.exists(os.path.join(context.device_broker.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.device_broker.compose_dir, "creds.example.env"),
            os.path.join(context.device_broker.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.device_broker.python_ver
    dockerfile_path = os.path.join(context.device_broker.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.device_broker.python_ver}"
        try:
            ...
            command = (
                f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.device_broker.nautobot_ver}"
            )
            if constrain_python_ver:
                command += f" --python {context.device_broker.python_ver}"
            run_command(context, command)
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations device_broker"
...
@task(
    help={
        "version": "Version of Device Broker to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = f"{base_pylint_command} device_broker"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("device_broker") / Path("migrations")
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.device_broker.local):
